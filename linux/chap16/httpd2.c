...
static char* lookup_header_filed_value(struct HTTPRequest *req, char *name);
...
static struct HTTPHeaderField* read_header_field(FILE *in)
{
    ...
    p = strchr(buf, ':');
    if (!p) log_exit("parse error on request header filed: %s", buf);
    *p++ = '\0';
...
content_length(struct HTTPRequest *req)
{
    ...
    val = lookup_header_filed_value(req, "Content-Length");
    if (!val) return 0;
    len = atoi(val);
...
static struct FileInfo* get_fileinfo(char *docroot, char *uripath)
{
    struct FileInfo *info;
    struct stat st;

    info = xmalloc(sizeof(struct FileInfo));
    info->path = build_fspath(docroot, uripath);
    info->ok = 0;
...
static void do_file_response(struct HTTPRequest *req, FILE *out, char *docroot)
{
    ...
    output_common_header_fields(req, out, "200_OK");
...
not_found(struct HTTPRequest *req, FILE *out)
{
    ...
    if (strcmp(req->method, "HEAD") != 0) {
        fprintf(out, "<html>\r\n");
        fprintf(out, "<header><tilte>Not Found</title><header>\r\n");
...
not_implemented(struct HTTPRequest *req, FILE *out)
{
    ...
    fprintf(out, "<html>\r\n");
    fprintf(out, "<header>\r\n");
    fprintf(out, "<tilte>501 Not Implemented</title>\r\n");
    fprintf(out, "<header>\r\n");
    fprintf(out, "<body>\r\n");
