...
{
public:
    employee(string_view name, unique_ptr<role> ownrole)
...
int main() {
    ...
    auto cecil = make_shared<employee>("cecil wiilamson", make_unique<president_role>());

    john->set_direct_manager(robert);
    robert->set_direct_manager(david);
    david->set_direct_manager(cecil);

    john->approve(expense{500, "magazine"});
