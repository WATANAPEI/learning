...
public:
   string to_base64(vector<unsigned char> const& data);
   vector<unsigned char> from_base64(string data);
};

string encoder::to_base64(vector<unsigned char> const &data) {
    string result;
...
vector<unsigned char> encoder::from_base64(string data)
{
    ...
    while (j++ < data.size() / 4)
    {
        ...
        if (c1 == invalid_char || c2 == invalid_char || c3 == invalid_char || c4 == invalid_char)
            throw runtime_error("invalid base64 encoding");
...
struct converter
{
    static vector<unsigned char> from_string(string_view data)
    {
        vector<unsigned char> result;
        copy(cbegin(data), cend(data), back_inserter(result));
        return result;
    }
    static string from_range(vector<unsigned char> const& data)
    {
        string result;
        copy(cbegin(data), cend(data), back_inserter(result));
...
int main() {
    vector<vector<unsigned char>> const data
    {
        {'s'},
        {'s', 'a'},
        {'s', 'a', 'm'},
